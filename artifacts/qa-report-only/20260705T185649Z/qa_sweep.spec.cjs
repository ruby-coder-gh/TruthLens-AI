...
test('qa report-only sweep', async ({ page, context }) => {
  ...
  // 3) workspace/chat access while logged out
  ...
  // 5) admin access control logged out
  ...
  // 6) admin access control as non-admin (if possible)
  ...
  if (!signupNav.ok) {
    ...
    const textInput = page.locator('input[name*="username" i], input[placeholder*="username" i], input[name*="name" i]').first();
    ...
    if (!emailFound || passwordCount < 1) {
      ...
      if (!loggedIn) {
        // try login with newly created credentials
      ...
      if (!loggedIn) {
        ...
        const deniedAsUser = looksDenied('/admin', page.url(), adminAsUserText) || /forbidden|not authorized|access denied/i.test(adminAsUserText);
        addCheck('admin_access_non_admin', {
