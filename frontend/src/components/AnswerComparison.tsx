...
export default function AnswerComparison({ comparison }: { comparison: QueryComparison }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="p-4"><div className="mb-3 flex items-center justify-between"><h3 className="font-medium text-text">Original answer</h3><Badge color={getTrustBadgeColor(comparison.original.trust_score ?? 0)}>{comparison.original.trust_score == null ? 'Unscored' : `${(comparison.original.trust_score * 100).toFixed(0)}% trust`}</Badge></div><p className="whitespace-pre-wrap text-sm leading-relaxed text-text-muted"><DiffText original={comparison.original.response_text || ''} rerun={comparison.rerun.response_text || ''} side="original" /></p></Card>
      <Card className="border-primary/30 p-4"><div className="mb-3 flex items-center justify-between"><h3 className="font-medium text-text">Current rerun</h3><Badge color={getTrustBadgeColor(comparison.rerun.trust_score ?? 0)}>{comparison.rerun.trust_score == null ? 'Unscored' : `${(comparison.rerun.trust_score * 100).toFixed(0)}% trust`}</Badge></div><p className="whitespace-pre-wrap text-sm leading-relaxed text-text-muted"><DiffText original={comparison.original.response_text || ''} rerun={comparison.rerun.response_text || ''} side="rerun" /></p></Card>
    </div>
