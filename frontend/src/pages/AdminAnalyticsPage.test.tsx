...
import type { UsageReportResponse, PricingResponse, GoldenListResponse } from '../api/types';
...
vi.mock('../api/client', () => ({
  adminApi: {
    ...
    exportUsage: vi.fn(),
    // L13 (golden-set inventory line in the eval tab) fires on mount for every
    // tab, so the usage-tab suite has to stub it too.
    getGolden: vi.fn(),
  },
}));
...
const GOLDEN: GoldenListResponse = {
  data: [],
  meta: {
    page: 1,
    page_size: 1,
...
describe('AdminAnalyticsPage — Usage tab', () => {
  beforeEach(() => {
    ...
    mockedAdminApi.getGolden.mockResolvedValue(GOLDEN);
  });
  ...
  // Integration guard: the L4 usage tab and the L13 golden-set line live in the
  // same tab chain — adding one must not hide the other.
  it('still renders the golden-set line on the RAGAS tab', async () => {
    renderWithProviders(<AdminAnalyticsPage />);

    await userEvent.click(await screen.findByRole('tab', { name: /ragas/i }));

    expect(
      await screen.findByText(/Golden set: builtin 12 \/ promoted 3/),
    ).toBeInTheDocument();
  });
});
